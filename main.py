...
from ToFPipeline import NXSLoader, PeakFinder, GlobalConfig
...
def process_detector_chunk(args):
    """Process detector data in a separate process - data is already normalized"""
    detector_data, config, worker_id = args
    try:
        # detector_data is passed directly (xarray DataArrays are picklable)
        # Note: Data is already normalized before chunking, so we skip normalize()
        pf = PeakFinder(detector_data, config=config)
        # stack() averages across trains/pulses based on config, then find peaks
        pf.stack().process()
            ...
            try:
                ...
                for i in range(self.n_detectors):
                    ...
                    try:
                        ...
                        # For stacked/averaged data, use the first (or only) pulse
                        trace = None
                        if normalized_data:
                        ...
                        if trace is None:
                            # For stacked data in the original buffer, also use first pulse
                            try:
                                trace = data.sel(detector=det_id).isel(pulse=0)
                            except:
                                # If that fails, try with last_train/last_pulse
                                trace = data.sel(
                                    detector=det_id, 
                                    pulse={'trainId': last_train, 'pulseId': last_pulse}
                                )
                            
                            # Normalize if using raw data
                            trace_max = trace.max().values
                            if trace_max > 0:
                                trace = trace / trace_max
                        
                        # Apply ROI slicing to limit plotted region
                        try:
                            roi_start, roi_end = self.roi
                            trace = trace.sel(sample=slice(roi_start, roi_end))
                        except:
                            pass  # If ROI slicing fails, use full trace
...
class FastMplCanvas(FigureCanvasQTAgg):
    ...
    def __init__(self, parent=None, width=5, height=4, dpi=100, n_detectors=16):
        ...
        for i in range(n_detectors):
            ...
            ax.set_ylim([-0.1, 1.1])
            # Format x-axis to show integers (sample indices)
...
class MainWindow(QMainWindow):
    ...
    def stack_buffer_data(self):
        ...
        # Compute all trains to numpy (avoid dask overhead)
        computed_trains = []
        for train in trains:
            if hasattr(train.data, 'compute'):
                computed_trains.append(train.compute())
            else:
                computed_trains.append(train)
    ...
    def on_tab_changed(self, index):
        """Handle tab changes - update results table when Results tab is selected"""
        if index == 1 and self.results_need_update:  # Results tab
            self.update_results_table()
